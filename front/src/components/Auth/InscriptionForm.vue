<script setup lang="ts">
    ...
    const submitForm = async () => {
        ...
        if(!isEmailValid(props.dataRegister.emailRegister.value)){
            props.errorRegister.emailRegister.value = "Adresse email invalide";
            return;
        }
        if(!isPasswordValid(props.dataRegister.passwordRegister.value)){
            props.errorRegister.passwordRegister.value = "Le mot de passe doit contenir au moins 8 caractères, un chiffre et un caractère spécial";
            return;
        }
        if(!PasswordMatching(props.dataRegister.passwordRegister.value,props.dataRegister.passwordComfirmation.value)){
            props.errorRegister.passwordComfirmation.value = "Les mots de passe ne correspondent pas";
            return;
        }
        if(!agreeToTerms.value){
            agreeError.value = "Vous devez accepter les conditions d'utilisation et la politique de confidentialité";
            return;
        }
...
<template>
    <div class="m-8 p-6 bg-white shadow-lg rounded-lg lg:max-w-xl w-full">
        <div class="flex flex-col items-center gap-1 mb-4">
            ...
            <p class="text-sm">Rejoignez-nous pour organiser les tâches de votre foyer</p>
        </div>
        <form class="flex flex-col gap-2" @submit.prevent="submitForm">
            ...
            <Field title="Adresse email" name="email" type="email" placeholder="Entrez votre email" v-model="props.dataRegister.emailRegister.value" icon="pi-at" :errorMessage="props.errorRegister.emailRegister.value"/>
