<script setup lang="ts">
    ...
    const submitForm = async () => {
        ...
        if(!isEmailValid(props.dataConnexion.emailConnexion.value)){
            props.errorConnexion.emailConnexion.value = "Adresse email invalide";
            return;
        }
        if(!isPasswordValid(props.dataConnexion.passwordConnexion.value)){
            props.errorConnexion.passwordConnexion.value = "Le mot de passe doit contenir au moins 8 caractères, un chiffre et un caractère spécial";
            return;
        }
...
<template>
    <div class="m-8 p-6 bg-white shadow-lg rounded-lg lg:max-w-xl w-full">
        <div class="flex flex-col items-center gap-1 mb-4">
            ...
            <h1 class="font-bold text-lg">Bon retour parmi nous</h1>
            <p class="text-sm">Connectez-vous pour gérer votre foyer</p>
        </div>
        <form class="flex flex-col gap-2" @submit.prevent="submitForm">
            <Field title="Adresse email" name="email" type="email" placeholder="Entrez votre email" v-model="props.dataConnexion.emailConnexion.value" icon="pi-at" :errorMessage="props.errorConnexion.emailConnexion.value"/>
            ...
            <p class="font-semibold text-md text-green_pastel text-center cursor-pointer" @click="toggleDisplay">Créer un compte</p>
        </form>
